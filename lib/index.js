    ...
    const processRaw = item => {
      ...
      if (this.validateCreate) {
        if (!this.validateCreate(item)) {
          ...
          validateErrors.push(...this.validateCreate.errors.map(e => e.message));
        }
      }
  ...
  patch (id, data, params) {
    ...
    if (this.validatePatch) {
      if (!this.validatePatch(data)) {
        throw new Error(`Data failed validation: ${this.validatePatch.errors.map(e => e.message).join(', ')}`);
      }
    }
  ...
  update (id, data, params) {
    ...
    if (this.validateCreate) {
      if (!this.validateCreate(entry)) {
        throw new Error(`Data failed validation: ${this.validateCreate.errors.map(e => e.message).join(', ')}`);
      }
    }
