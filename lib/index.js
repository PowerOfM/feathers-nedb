...
class Service {
  ...
  create (raw, params) {
    ...
    const processRaw = item => {
      ...
      if (this.addTimestamps) {
        item[this.createdTimestamp] = new Date();
        item[this.updatedTimestamp] = new Date();
      }

      if (this.validateCreate) {
        if (!this.validateCreate(item)) {
          validateErrors = validateErrors || []
          validateErrors.push(this.validateCreate.errors);
        }
      }
  ...
  patch (id, data, params) {
    ...
    if (this.addTimestamps) {
      data[this.updatedTimestamp] = new Date();
    }

    if (this.validatePatch) {
      if (!this.validatePatch(data)) {
        throw new Error(`Data failed validation: ${this.validatePatch.errors}`);
      }
    }
  ...
  update (id, data, params) {
    ...
    if (this.addTimestamps) {
      entry[this.updatedTimestamp] = new Date();
    }

    if (this.validatePatch) {
      if (!this.validatePatch(entry)) {
        throw new Error(`Data failed validation: ${this.validatePatch.errors}`);
      }
    }
